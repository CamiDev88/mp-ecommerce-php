<?php  
    ...
    try{
        ...
        // IMPORTANT
        // This doesn't work on "modal" scheme (it works in redirect - TESTED) !!!!
        $preference->back_urls = array(
            "success" => SERVER."/payment-result.php?status=success",
            "failure" => SERVER."/payment-result.php?status=failure",
            "pending" => SERVER."/payment-result.php?status=pending"
        );
        ...
        $preference->notification_url = SERVER."/webhooks.php";

        $preference->save();

        if(!$preference->id){
            $error = 'Ocurrió un error inesperado, por favor intente en unos instantes.';
        }
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
